...
- WireGuard first parity slice: DB-backed clients/server config, key generation, add/delete/toggle, reply flows for rename/timer/DNS/MTU/AllowedIPs, Telegram config document upload, QR image upload, Amnezia toggle/key material, richer menu status, `wg0.conf`/`wg1.conf` rendering, optional reload via `WG_RELOAD=1`.
...
- WireGuard remaining: global subnet list editor and live traffic/handshake display.
